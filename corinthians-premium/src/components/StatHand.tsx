...
const StatHand = ({
  ...
  useEffect(() => {
    ...
    gsap.to(statsValue, {
      ...
      onUpdate: () => {
        if (numberRef.current) {
          numberRef.current.innerText = Math.floor(statsValue.val).toString();
        }
      },
    });
    ...
    const handleMouse = (e: MouseEvent) => {
      ...
      if (Math.abs(dist) < 300) {
        const rotationAmount = (dist / 300) * -15;
  ...
  return (
    <div
      ref={containerRef}
      className="relative flex flex-col items-center justify-end h-[50vh] w-full origin-bottom"
    >
      <img
        src={`/hand${index}.png`}
      ...
      />
      <div
        className="absolute z-20 w-[75%] text-center"
        style={{ bottom: textOffset }}
      >
        <p className="text-black font-bold text-[8px] md:text-[10px] tracking-[0.2em] mb-1 opacity-70">
          {label}
        </p>
